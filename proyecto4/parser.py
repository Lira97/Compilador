...
def VarDeclaration():
    ...
    if(match(TokenType.SEMICOLON)):
        ...
        if(match(TokenType.NUM)):
            childNodes.append(Node("NUM",[],ListValues[currentToken]))
...
def Mulop():
    ...
    if(match(TokenType.ASTERISK)):#detectamos el *
        return(Node("*"))
    elif(ListOfTokens[currentToken] == TokenType.SLASH):#detectamos el /
        return(Node("/"))
    return None
...
def Factor():
    ...
    if(match(TokenType.OPEN_PARENTHESIS)):#detecta el (expresion )
        ...
        childNodes.append(Node("num", [],ListValues[currentToken]))#detecta el NUM
        return(Node("factor", childNodes))
